...
import captcha.filter.Filter;

public class SmartBlurFilter extends AbstractBufferedImageOp implements Filter {
    ...
    public BufferedImage filter(final String srcFile) throws IOException {
        ...
        final BufferedImage src = ImageIO.read(fin);
        final int width = src.getWidth();
        final int height = src.getHeight();

        final BufferedImage dst  = createCompatibleDestImage(src, null);

        final int[] inPixels = new int[width * height];
        final int[] outPixels = new int[width * height];
        getRGB(src, 0, 0, width, height, inPixels);

        final Kernel kernel = GaussianFilter.makeKernel(hRadius);
