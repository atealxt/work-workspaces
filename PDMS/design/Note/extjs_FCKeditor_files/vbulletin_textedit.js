...
function vB_Text_Editor(editorid, mode, parsetype, parsesmilies, initial_text, ajax_extra)
{
	...
	this.init_popup_button = function(obj)
	{
		...
		if (this.popupmode)
		{
			...
			if (obj.cmd == 'fontname')
			{
				...
				{
					if(!fontoptions.hasOwnProperty(option)){continue;}
			...
			else if (obj.cmd == 'fontsize')
			{
				...
				{
					if(!sizeoptions.hasOwnProperty(option)){continue;}
					var div = document.createElement('div');
	...
	this.build_select = function(obj)
	{
		...
		switch (obj.cmd)
		{
			...
			case 'forecolor':
			{
				...
				{
					if(!coloroptions.hasOwnProperty(i)){continue;}
					var opt = document.createElement('option');
			...
			case 'smilie':
			{
				...
				{
					if(!smilieoptions.hasOwnProperty(cat)){continue;}
					for (var smilieid in smilieoptions[cat])
					{
						if(!smilieoptions[cat].hasOwnProperty(smilieid)){continue;}
						if (smilieid != 'more')
						{
							var opt = document.createElement('option');
	...
	*/
	this.build_fontname_popup = function(obj, menu)
	{
		...
		{
			if(!fontoptions.hasOwnProperty(n)){continue;}
	...
	*/
	this.build_fontsize_popup = function(obj, menu)
	{
		...
		{
			if(!sizeoptions.hasOwnProperty(n)){continue;}
			var option = document.createElement('div');
	...
	*/
	this.build_forecolor_popup = function(obj, menu)
	{
		...
		{
			if(!coloroptions.hasOwnProperty(hex)){continue;}
			if (i % 8 == 0)
			{
	...
	*/
	this.build_attachments_popup = function(menu, obj)
	{
		if (this.popupmode)
		{
			...
			{
				if(!vB_Attachments.attachments.hasOwnProperty(id)){continue;}
				var div = document.createElement('div');
				div.editorid = this.editorid;
				div.controlkey = obj.id;
	...
	*/
	this.destroy = function()
	{
		...
		{
			if(!this.buttons.hasOwnProperty(i)){continue;}
		...
		{
			if (this.popups.hasOwnProperty(menu)) {
		...
		if (this.fontoptions)
		{
			...
			{
				if(!this.fontoptions.hasOwnProperty(i)){continue;}
				if (i != '')
				{
		...
		if (this.sizeoptions)
		{
			...
			{
				if(!this.sizeoptions.hasOwnProperty(i)){continue;}
				if (i != '')
				{
...
/*======================================================================*\
|| ####################################################################
|| # Downloaded: 10:45, Tue May 29th 2007
